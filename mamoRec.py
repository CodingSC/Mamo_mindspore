...
class MAMRec:
    def __init__(self, dataset='movielens'):
        ...
        mindspore.context.set_context(device_target=self.device)
        print(self.device)
        ...
        if dataset == 'movielens':
            self.x1_loading, self.x2_loading = MLUserLoading(embedding_size=self.embedding_dim, name='user_loading'), \
                                               MLItemLoading(embedding_size=self.embedding_dim, name='item_loading')
        else:
            self.x1_loading, self.x2_loading = BKUserLoading(embedding_size=self.embedding_dim, name='user_loading'), \
                                               BKItemLoading(embedding_size=self.embedding_dim, name='item_loading')
        ...
        self.UEmb = UserEmbedding(self.n_layer, default_info[dataset]['u_in_dim'] * self.embedding_dim,
                                  self.embedding_dim, activation=self.active_func, name='user_embed')
        self.IEmb = ItemEmbedding(self.n_layer, default_info[dataset]['i_in_dim'] * self.embedding_dim,
                                  self.embedding_dim, activation=self.active_func, name='item_embed')


        # rec model
        self.rec_model = RecMAM(self.embedding_dim, self.n_y, self.n_layer, activation=self.active_func, name='rec')


        # whole model
        self.model = BASEModel(self.x1_loading, self.x2_loading, self.UEmb, self.IEmb, self.rec_model)
    ...
    def train_with_meta_optimization(self):
        ...
        for i in range(self.n_epoch):
            ...
            with open('time.txt', 'a') as f:  # 以追加模式打开文件，如果文件不存在会自动创建
                f.write('Epoch {} cost {:.2f}s\n'.format(i+1, end-start))
